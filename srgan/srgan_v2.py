...
class SRGAN():
    def __init__(self):
        ...
        self.vgg1 = self.build_vgg(self.hr_shape)
        self.vgg1.trainable = False
        self.vgg1.compile(loss='mse',
            optimizer=optimizer,
            metrics=['accuracy'])
        ...
        # self.discriminator.load_weights('srgan-weights/dis_srgan.h5')
        ...
        # self.generator.load_weights('srgan-weights/gen_srgan.h5')
        ...
        fake_features, low_fake_features = self.vgg1(fake_hr)
        ...
        self.combined = Model(img_lr, [validity, fake_features, low_fake_features])
        self.combined.compile(loss=['binary_crossentropy', self.mse, self.mse],
                              loss_weights=[1e-3, 0, 1],
                              optimizer=optimizer)
    ...
    def build_vgg(self, shape):
        ...
        out1 = vgg.layers[9].output
        out2 = vgg.layers[5].output
        vgg.outputs = [out1, out2]

        img = Input(shape=input_shape)

        # Extract image features
        img_features, low_img_features = vgg(img)
        return Model(img, [img_features, low_img_features])
    ...
    def train(self, epochs, batch_size=1, sample_interval=500):
        ...
        for epoch in range(epochs):
            ...
            image_features, low_image_features = self.vgg1.predict(imgs_hr)

            # Train the generators
            g_loss = self.combined.train_on_batch(imgs_lr, [valid, image_features, low_image_features])
